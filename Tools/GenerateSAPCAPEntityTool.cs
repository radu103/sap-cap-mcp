...
namespace SAPCAPTools.Tools
{
    ...
    public static class GenerateSAPCAPEntityTool
    {
        ...
        [McpServerTool, Description("Generate SAP CDS DB Entity")]
            ...
            [Description("Existing Entities in Database Schema are mandatory for Associations")] List<string> existingEntities)
        {
            ...
            try
            {
                ...
                foreach (var association in entityAssociations)
                {
                    ...
                    sb.AppendLine($"  {associationField + "ID".PadRight(maxLength)} : UUID;");
                    sb.AppendLine($"  {associationField.PadRight(maxLength)} : Association to {associationEntity} on {associationField}.ID = $self.{associationField}ID;");
                }
